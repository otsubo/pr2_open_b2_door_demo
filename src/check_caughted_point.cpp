...
void calculate(
               ...
               const visualization_msgs::MarkerArray::ConstPtr& marker_array)
{
  //std::count<<"calculate"<<std::endl;
  jsk_recognition_msgs::BoundingBox new_bbox_msg(*box);
  //visualization_msgs::MarkerArray ma;
  visualization_msgs::MarkerArray new_marker_msg(*marker_array);
...
main(int argc, char** argv)
{
  ...
  ros::NodeHandle nh;
  typedef message_filters::sync_policies::ApproximateTime<jsk_recognition_msgs::BoundingBox, visualization_msgs::MarkerArray> MySyncPolicy;
  ...
  message_filters::Subscriber<visualization_msgs::MarkerArray> marker_sub(nh, "input_marker",1);
  ...
  sync_.connectInput(box_sub, marker_sub);
  sync_.registerCallback(boost::bind(&calculate, _1, _2));

  //message_filters::Synchronizer<MySyncPolicy> sync_(MySyncPolicy(10), box_sub, marker_sub);
  //sync.registerCallback(boost::bind(&calculate, _1, _2));
