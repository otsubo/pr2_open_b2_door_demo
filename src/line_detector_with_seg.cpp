...
#include <jsk_recognition_msgs/SegmentStamped.h>
...
jsk_recognition_msgs::SegmentStamped::Ptr make_line_marker(const pcl::PointCloud<pcl::PointXYZ>::Ptr& cloud, const pcl::PointIndices::Ptr& inliers, const pcl::ModelCoefficients::Ptr& coefficients,int count, bool refined){
  ...
  jsk_recognition_msgs::Segment::Ptr segment (new jsk_recognition_msgs::Segment);
  //segment->header = pcl_conversions::fromPCL(cloud->header);
  // if (refined){
  //   segment->ns = "refined_line_segment";}
  // else{
  //   segment->ns = "line_segment";}
  // segment->id = count;
  // segment->type = visualization_msgs::Marker::LINE_LIST;

  // segment_stamped->points.push_back(p0);
  // segment_stamped->points.push_back(p1);
  segment->start_point = p0;
  segment->end_point = p1;
...
cloud_cb (const sensor_msgs::PointCloud2ConstPtr& cloud_msg)
{
  ...
  jsk_recognition_msgs::SegmentArray segment_array;
  ...
  // Publish the data
      pub.publish (output);
      segment_arr_pub.publish (segment_array);
...
main (int argc, char** argv)
{
  ...
  segment_arr_pub = nh.advertise<jsk_recognition_msgs::SegmentArray> ("segments_output", 1);
